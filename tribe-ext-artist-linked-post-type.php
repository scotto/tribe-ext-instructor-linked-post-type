<?php
/**
 * Plugin Name:       The Events Calendar Extension: Artist Linked Post Type
 * Plugin URI:        https://theeventscalendar.com/knowledgebase/linked-post-types/
 * GitHub Plugin URI: https://github.com/scotto/tribe-ext-artist-linked-post-type
 * Description:       An "Artist" custom post type will be linked to The Events Calendar's Events, like Organizers are, and basic output will be added to the Single Event Page.
 * Version:           1.0.1
 * Extension Class:   Tribe__Extension__Artist_Linked_Post_Type
 * Author:            Scott Hepler, originally Modern Tribe, Inc.
 * Author URI:        http://scotthepler.com
 * License:           GPL version 3 or any later version
 * License URI:       https://www.gnu.org/licenses/gpl-3.0.html
 * Text Domain:       tribe-ext-artist-linked-post-type
 */
 ...
 * Test everything is working as desired, including saving of meta data and customizing how it outputs to the Single Events Page and your single artist page.
 */

if ( ! function_exists( 'tribe_ext_is_event_artist' ) ) {
	/**
	 * Conditional tag to check if the current page is a single artist page.
	 *
	 * @return bool
	 **/
	function tribe_ext_is_event_artist() {
		global $wp_query;

		$tribe_ext_is_event_artist = ! empty( $wp_query->tribe_ext_is_event_artist );

		return apply_filters( 'tribe_ext_query_is_event_artist', $tribe_ext_is_event_artist );
	}
}
...
if (
	...
	&& ! class_exists( 'Tribe__Extension__Artist_Linked_Post_Type' )
) {
	/**
	 * Extension main class, class begins loading on init() function.
	 */
	class Tribe__Extension__Artist_Linked_Post_Type extends Tribe__Extension {
		 ...
		 * @return string
		 */
		// const POST_TYPE_KEY = 'tribe_ext_artist';
		const POST_TYPE_KEY = 'tribe_artist';
		...
		public function init() {
			...
			load_plugin_textdomain( 'tribe-ext-artist-linked-post-type', false, basename( dirname( __FILE__ ) ) . '/languages/' );
			...
			// Single Artist page: Handling the No Events Found situation.
		...
		public function add_filter_to_filterbar() {
			if ( class_exists( 'Tribe__Events__Filterbar__Filters__Artist' ) ) {
				new Tribe__Events__Filterbar__Filters__Artist( $this->get_post_type_label(), self::POST_TYPE_SLUG );
			}
		}
		 ...
		 *
		 * @see Tribe__Extension__Artist_Linked_Post_Type::sanitize_a_custom_fields_value()
		 *
		 * @return array
		 */
		 ...
		 *
		 * @see Tribe__Extension__Artist_Linked_Post_Type::get_custom_field_labels()
		 * @see sanitize_key()
		 *
		 * @return array
		 */
		...
		public function register_our_post_type() {
			$labels = array(
				'name'                    => esc_html_x( 'Artists', 'Post type general name', 'tribe-ext-artist-linked-post-type' ),
				'singular_name'           => esc_html_x( 'Artist', 'Post type singular name', 'tribe-ext-artist-linked-post-type' ),
				'singular_name_lowercase' => esc_html_x( self::POST_TYPE_SLUG, 'Post type singular name', 'tribe-ext-artist-linked-post-type' ),
				// not part of WP's labels but is required by Linked_Posts::register_linked_post_type()
				'add_new'                 => esc_html_x( 'Add New', self::POST_TYPE_KEY, 'tribe-ext-artist-linked-post-type' ),
				'add_new_item'            => esc_html__( 'Add New Artist', 'tribe-ext-artist-linked-post-type' ),
				'edit_item'               => esc_html__( 'Edit Artist', 'tribe-ext-artist-linked-post-type' ),
				'new_item'                => esc_html__( 'New Artist', 'tribe-ext-artist-linked-post-type' ),
				'view_item'               => esc_html__( 'View Artist', 'tribe-ext-artist-linked-post-type' ),
				'view_items'              => esc_html__( 'View Artists', 'tribe-ext-artist-linked-post-type' ),
				'search_items'            => esc_html__( 'Search Artists', 'tribe-ext-artist-linked-post-type' ),
				'not_found'               => esc_html__( 'No artists found', 'tribe-ext-artist-linked-post-type' ),
				'not_found_in_trash'      => esc_html__( 'No artists found in Trash', 'tribe-ext-artist-linked-post-type' ),
				'all_items'               => esc_html__( 'Artists', 'tribe-ext-artist-linked-post-type' ),
				'archives'                => esc_html__( 'Artist Archives', 'tribe-ext-artist-linked-post-type' ),
				'insert_into_item'        => esc_html__( 'Insert into artist', 'tribe-ext-artist-linked-post-type' ),
				'uploaded_to_this_item'   => esc_html__( 'Uploaded to this artist', 'tribe-ext-artist-linked-post-type' ),
				'items_list'              => esc_html__( 'Artists list', 'tribe-ext-artist-linked-post-type' ),
				'items_list_navigation'   => esc_html__( 'Artists list navigation', 'tribe-ext-artist-linked-post-type' ),
			);

			$args = array(
				'labels'              => $labels,
				'description'         => esc_html__( 'Artists linked to Events', 'tribe-ext-artist-linked-post-type' ),
				'public'              => true,
		...
		public function get_post_id_field_name() {
			return $this->get_post_type_label( 'singular_name' ) . '_ID'; // Artist_ID
		}
		...
		private function get_order_meta_key() {
			$key = '_' . $this->get_post_id_field_name() . '_Order'; // _Artist_ID_Order

			return sanitize_key( $key ); // _artist_id_order
		}
		...
		public function add_meta_box_to_event_editing() {
			...
			add_meta_box(
				$meta_box_id,
				sprintf( esc_html__( '%s Information', 'tribe-ext-artist-linked-post-type' ), $this->get_post_type_label( 'singular_name' ) ),
				array( $this, 'meta_box' ),
				self::POST_TYPE_KEY,
				'normal',
				'high'
		...
		protected function get_meta_box_tr_html_for_a_field_label( $custom_field_label, $post_id = 0 ) {
			...
			<input id="%3$s" type="text"
				...
				$custom_field_key,
				esc_html__( $custom_field_label . ':', 'tribe-ext-artist-linked-post-type' ),
				$name,
				esc_html( $value )
			);
		...
		/**
		 * Saves the artist information passed via an event.
		 *
		...
		public function create_new_post( $data, $post_status = 'publish' ) {
			...
			if (
				...
				$title   = isset( $data[$name_field_index] ) ? $data[$name_field_index] : sprintf( esc_html__( 'Unnamed %s', 'tribe-ext-artist-linked-post-type' ), $this->get_post_type_label( 'singular_name' ) );
				...
				$artist_id = wp_insert_post( array_filter( $postdata ), true );

				if ( ! is_wp_error( $artist_id ) ) {
					$this->save_meta( $artist_id, $data );
		 ...
		 *
		 * @see Tribe__Extension__Artist_Linked_Post_Type::event_edit_form_save_data()
		 * @see Tribe__Extension__Artist_Linked_Post_Type::save_data_from_meta_box()
		 *
		 * @param int   $id   The artist ID to update.
		 * @param array $data The artist data.
		 *
		 * @return int The updated artist post ID
		 */
		...
		public function set_post_type_in_parse_query( $query ) {
			...
			if (
				...
				$query->tribe_ext_is_event_artist = true;
				...
				// Single Artist Page: Force Previous and Next navigation links to not appear.
		...
		public function set_event_query_for_this_single_post( $tribe_is_event_query ) {
			...
			if ( ! empty( $wp_query->tribe_ext_is_event_artist ) ) {
				return true;
			} else {
		...
		/**
		 * Override the Previous Events link on the Single Artist page.
		 *
		 * @param $link
		 *
		 * @return string
		...
		/**
		 * Override the Next Events link on the Single Artist page.
		 *
		 * @param $link
		 *
		 * @return string
		...
		protected function nothing_found_notice() {
			Tribe__Notices::set_notice( 'event-search-no-results', esc_html__( 'There were no results found.', 'tribe-ext-artist-linked-post-type' ) );
		}
